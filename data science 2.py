...
def answer_one():
    ...
    #ScimEnTop15 = ScimEn[ScimEn['Rank']<=15]
    
    #GDPDrop = GDP.drop(labels=[str(x) for x in range(1960,2006)],axis=1)
    #GDPDrop = GDPDrop.drop(labels=['Country Code','Indicator Name','Indicator Code'],axis=1)
    ...
    merge2.drop(labels=[str(x) for x in range(1960,2006)],axis=1,inplace=True)
...
def answer_three():
    Top15 = answer_one()
    rows = [str(x) for x in range(2006,2016)]
